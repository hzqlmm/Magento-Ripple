...
class Appmerce_Ripple_Model_Process extends Varien_Object
{
    /**
    ...
    public function invoice($order)
    {
        ...
        if (!$order->hasInvoices() && $order->canInvoice()) {
            ...
            if ($invoice->getTotalQty() > 0) {
                ...
                $transaction->save();
                $invoice->addComment(Mage::helper('ripple')->__('Automatic invoice.'), false);
    ...
    {
        $this->restore();
        $message = Mage::helper('ripple')->__('Payment failed. Please try again.');
    ...
    {
        $this->getCheckout()->setQuoteId($this->getCheckout()->getRippleQuoteId(true));
        $this->getCheckout()->setLastOrderId($this->getCheckout()->getRippleOrderId(true));
    }
